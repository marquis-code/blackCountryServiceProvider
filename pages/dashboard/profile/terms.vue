<template>
  ...
  <main>
    ...
    <section v-else class="flex px-6 flex-col border-[0.5px] border-gray-100 py-10 rounded-lg  justify-between items-center space-y-2 mt-10">
      ...
      <h2 class="text-[#1D2739]">No Terms available</h2>
    </section>
  </main>
</template>
...
const formattedTermsList = computed(() => {
  if (!termsList.value || !termsList.value.content) return "";
  const content = termsList.value.content; // Assume the backend returns a content string in HTML format
  return `
      <html>
        <head>
          <style>
